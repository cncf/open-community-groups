...
fn build_event_calendar_description(event: &EventSummary) -> String {
    ...
    if let Some(meeting_url) = event.meeting_url.as_deref().filter(|url| !url.trim().is_empty()) {
        description.push(format!("Meeting link: {meeting_url}"));
    }
...
mod tests {
    ...
    fn sample_event(canceled: bool) -> EventSummary {
        EventSummary {
            ...
            meeting_url: Some("https://example.test/live".to_string()),
            popover_html: None,
