...
async fn test_list_page_db_error() {
    ...
    db.expect_list_alliance_audit_logs()
        ...
        .withf(move |id, filters| {
            *id == alliance_id && filters.limit == Some(50) && filters.offset == Some(0)
        })
...
async fn test_list_page_success() {
    ...
    db.expect_list_alliance_audit_logs()
        ...
        .withf(move |id, filters| {
            *id == alliance_id
                && filters.action.as_deref() == Some("alliance_updated")
    ...
    let request = Request::builder()
        .method("GET")
        .uri(
            "/dashboard/alliance/logs?action=alliance_updated&actor=test-user\
    ...
    assert_eq!(
        ...
        &HeaderValue::from_static(concat!(
            "/dashboard/alliance?tab=logs&action=alliance_updated&actor=test-user",
            "&date_from=2024-01-01&date_to=2024-01-31&limit=5&offset=10&sort=created-asc",
        )),
    );
