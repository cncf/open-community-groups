...
async fn test_add_db_error() {
    ...
    db.expect_add_event_category()
        ...
        .withf(move |uid, cid, category| {
            *uid == user_id && *cid == alliance_id && category.name == "Conference"
        })
    ...
    let request = Request::builder()
        .method("POST")
        .uri("/dashboard/alliance/event-categories/add")
...
async fn test_delete_db_error() {
    ...
    db.expect_delete_event_category()
        .times(1)
        .withf(move |uid, cid, ecid| {
            *uid == user_id && *cid == alliance_id && *ecid == event_category_id
        })
        .returning(|_, _, _| Err(anyhow!("db error")));
    ...
    let request = Request::builder()
        .method("DELETE")
        .uri(format!(
            "/dashboard/alliance/event-categories/{event_category_id}/delete"
        ))
        .header(HOST, "example.test")
        .header(COOKIE, format!("id={session_id}"))
        .body(Body::empty())
...
async fn test_list_page_db_error() {
    ...
    let request = Request::builder()
        .method("GET")
        .uri("/dashboard/alliance/event-categories")
        .header(HOST, "example.test")
        .header(COOKIE, format!("id={session_id}"))
        .body(Body::empty())
        .unwrap();
...
async fn test_update_db_error() {
    ...
    let request = Request::builder()
        .method("PUT")
        .uri(format!(
            "/dashboard/alliance/event-categories/{event_category_id}/update"
        ))
        .header(HOST, "example.test")
        .header(COOKIE, format!("id={session_id}"))
        .header(CONTENT_TYPE, "application/x-www-form-urlencoded")
