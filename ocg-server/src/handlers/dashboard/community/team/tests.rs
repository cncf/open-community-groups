...
async fn test_add_db_error() {
    ...
    db.expect_get_community_summary().never();
...
async fn test_delete_success() {
    ...
    db.expect_delete_community_team_member()
        ...
        .withf(move |actor_user_id, cid, uid| {
            *actor_user_id == user_id && *cid == community_id && *uid == member_id
        })
...
async fn test_delete_current_user_logs_out() {
    ...
    db.expect_delete_community_team_member()
        .times(1)
        .withf(move |actor_user_id, cid, uid| {
            *actor_user_id == user_id && *cid == community_id && *uid == user_id
        })
        .returning(move |_, _, _| Ok(()));
    ...
    let request = Request::builder()
        .method("DELETE")
        .uri(format!("/dashboard/community/team/{user_id}/delete"))
        .header(HOST, "example.test")
