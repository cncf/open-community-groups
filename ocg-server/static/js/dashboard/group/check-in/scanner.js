...
/** Loads the selected event's attendee table without leaving the mobile check-in surface. */
const openManualCheckIn = async (root, link) => {
  ...
  content.textContent = "Loading attendees…";
  ...
  try {
    ...
    content.textContent = "The attendee list could not be loaded. Return to the scanner and try again.";
  } finally {
...
const populateCameras = async (select, scanner, isCurrent, torch) => {
  ...
  const changeHandler = async () => {
    ...
    try {
      ...
      if (isCurrent()) setTorchAvailable(torch, hasFlash);
    } catch {
...
const startScannerSession = async (root, trigger) => {
  ...
  const video = root.querySelector("[data-group-check-in-video]");
  ...
  if (!(modal instanceof HTMLElement) || !(video instanceof HTMLVideoElement)) return;
  ...
  try {
    ...
    if (camera instanceof HTMLSelectElement) {
      cleanupCameraSelection = await populateCameras(camera, scanner, isCurrent, torch);
    }
    if (!isCurrent()) {
      cleanupControls();
      controller.teardown();
    ...
    if (torch instanceof HTMLButtonElement && (await scanner.hasFlash()) && isCurrent()) {
      setTorchAvailable(torch, true);
      torchClickHandler = async () => {
        try {
