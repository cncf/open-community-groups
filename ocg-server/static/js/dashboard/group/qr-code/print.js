...
export const printQrCode = (modal, imageId, qrUrl) => {
  ...
  // Print a static clone so modal controls and live page state stay untouched.
  ...
  if (qrImage) {
    ...
    // Cache-bust the print image so the browser waits for the current QR code.
  ...
  // Wait for the QR image before printing; otherwise some browsers print a blank.
  const handleLoad = () => {
