...
} from "/static/js/alliance/explore/filters.js";
import {
  closestElement,
  getElementById,
  isDatasetReady,
...
export const initializeExploreWidgets = async (root = document, { force = false } = {}) => {
  ...
  if (calendarMarker && (force || !isDatasetReady(calendarMarker, EXPLORE_WIDGET_READY_KEY))) {
    ...
    if (data) {
      ...
      const module = await import("/static/js/alliance/explore/calendar.js");
  ...
  if (mapMarker && (force || !isDatasetReady(mapMarker, EXPLORE_WIDGET_READY_KEY))) {
    ...
    if (data && entity) {
      ...
      const module = await import("/static/js/alliance/explore/map.js");
