...
declare
    ...
    v_session_id uuid;
begin
    -- Case 1: Event needing create/update (not deleted/canceled)
    ...
    -- Case 2: Session needing create/update (parent event active)
    -- Priority: create/update operations come before deletes
    ...
    -- Case 3: Event needing delete (deleted or canceled, has meeting)
    select e.event_id into v_event_id
    from event e
    join meeting m on m.event_id = e.event_id
    where e.meeting_requested = true
      and e.meeting_in_sync = false
      and (e.deleted = true or e.canceled = true)
    ...
    -- Case 4: Session needing delete (parent event deleted/canceled, has meeting)
    select s.session_id into v_session_id
    from session s
    join event e on e.event_id = s.event_id
    join meeting m on m.session_id = s.session_id
    where s.meeting_requested = true
      and (e.deleted = true or e.canceled = true)
    for update of s skip locked
    limit 1;
