...
create or replace function update_user_details(
    ...
    set
        ...
        interests = case
            when p_user ? 'interests' and jsonb_typeof(p_user->'interests') = 'null' then null
            when p_user ? 'interests' then array(select jsonb_array_elements_text(p_user->'interests'))
            else interests
        end,
