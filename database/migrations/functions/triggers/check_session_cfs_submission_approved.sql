-- Requires session CFS links to reference an approved submission of the same event.
...
begin
    ...
    if v_event_id is null then
        raise exception 'cfs submission not found' using errcode = 'OCG01';
    end if;

    -- Reject links to submissions for another event
    if v_event_id <> new.event_id then
        raise exception 'cfs submission does not belong to the session event' using errcode = 'OCG01';
    end if;

    -- Reject links to submissions outside the approved state
    if v_status_id <> 'approved' then
        raise exception 'cfs submission must be approved' using errcode = 'OCG01';
    end if;
