...
begin
    ...
    if exists (
        ...
        raise exception 'discount codes must be unique per event' using errcode = 'OCG01';
    end if;
    ...
    loop
        if (v_discount_code->>'event_discount_code_id') is null then
            raise exception 'discount codes require event_discount_code_id' using errcode = 'OCG01';
        end if;

        if nullif(v_discount_code->>'code', '') is null then
            raise exception 'discount codes require code' using errcode = 'OCG01';
        end if;

        if nullif(v_discount_code->>'title', '') is null then
            raise exception 'discount codes require title' using errcode = 'OCG01';
        end if;

        if (v_discount_code->>'available') is not null
           and (v_discount_code->>'available')::int < 0
        then
            raise exception 'discount code available must be greater than or equal to 0' using errcode = 'OCG01';
        end if;

        if (v_discount_code->>'total_available') is not null
           and (v_discount_code->>'total_available')::int < 0
        then
            raise exception 'discount code total_available must be greater than or equal to 0' using errcode = 'OCG01';
        end if;
        ...
        then
            raise exception 'discount code available cannot exceed total_available' using errcode = 'OCG01';
        end if;
        ...
        then
            raise exception 'discount code ends_at cannot be before starts_at' using errcode = 'OCG01';
        end if;

        if v_discount_code->>'kind' = 'fixed_amount' then
            if (v_discount_code->>'amount_minor') is null then
                raise exception 'fixed amount discount codes require amount_minor' using errcode = 'OCG01';
            end if;

            if (v_discount_code->>'amount_minor')::bigint <= 0 then
                raise exception 'discount code amount_minor must be greater than 0' using errcode = 'OCG01';
            end if;

            if v_discount_code->>'percentage' is not null then
                raise exception 'fixed amount discount codes cannot include percentage' using errcode = 'OCG01';
            end if;
        elsif v_discount_code->>'kind' = 'percentage' then
            if (v_discount_code->>'percentage') is null then
                raise exception 'percentage discount codes require percentage' using errcode = 'OCG01';
            end if;

            if (v_discount_code->>'percentage')::int < 1
               or (v_discount_code->>'percentage')::int > 100
            then
                raise exception 'discount percentage must be between 1 and 100' using errcode = 'OCG01';
            end if;

            if v_discount_code->>'amount_minor' is not null then
                raise exception 'percentage discount codes cannot include amount_minor' using errcode = 'OCG01';
            end if;
        else
