-- Cancels an approved event co-hosting relationship.
create or replace function cancel_event_cohost(
...
begin
    ...
    -- Reject invitations outside the approved state
    if v_cohost.event_cohost_status_id <> 'approved' then
        raise exception 'co-hosting is not approved' using errcode = 'OCG01';
    end if;

    -- Persist the co-host cancellation while retaining approval evidence
    update event_cohost
    set
        event_cohost_status_id = 'canceled',
    ...
    -- Audit the canceled transition for both groups
    perform insert_event_cohost_audit(
        'event_cohost_canceled',
        p_actor_user_id,
        v_event.event_id,
        v_event.group_id,
        p_cohost_group_id,
