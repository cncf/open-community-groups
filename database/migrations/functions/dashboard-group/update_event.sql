    ...
    if p_event->'sessions' is not null then
        ...
        loop
            ...
            if v_session->>'session_id' is not null then
                ...
                from json_each(v_event_before->'sessions') as day(day, sessions)
                cross join lateral json_array_elements(sessions) as sess
                ...
                update session set
                    ...
                    meeting_in_sync = (select is_session_meeting_in_sync(v_session_before, v_session, v_event_before->>'timezone', p_event->>'timezone')),
