...
create index audit_log_alliance_id_created_at_idx on audit_log (alliance_id, created_at desc);
...
drop function if exists accept_alliance_team_invitation(uuid, uuid);
...
drop function if exists add_alliance_team_member(uuid, uuid, uuid, text);
...
drop function if exists delete_alliance_team_member(uuid, uuid, uuid);
...
drop function if exists reject_alliance_team_invitation(uuid, uuid);
...
drop function if exists update_alliance(uuid, uuid, jsonb);
drop function if exists update_alliance_team_member_role(uuid, uuid, uuid, text);
