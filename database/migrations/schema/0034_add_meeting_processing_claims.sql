-- Adds meeting processing claims and resets affected function signatures.

-- Drop function signatures replaced by claim-aware implementations
...
-- Add synchronization claims to event and session ownership rows
alter table event
...
-- Add processing claims to provider meeting rows
alter table meeting
    add column auto_end_check_claimed_at timestamptz,
    add column sync_claimed_at timestamptz;

-- Index claim selection paths for events, meetings, and sessions
