...
-- Should succeed when host is from same community as event
select lives_ok(
    format('insert into event_host (event_id, user_id) values (%L, %L)', :'eventID', :'user1ID'),
    'Should succeed when host is from same community as event'
);

-- Should fail when host is from different community
select throws_ok(
    format('insert into event_host (event_id, user_id) values (%L, %L)', :'eventID', :'user2ID'),
    format('host user %s not found in community', :'user2ID'),
    'Should fail when host is from different community'
);

-- Should fail when updating event_host to user from different community
select throws_ok(
    format('update event_host set user_id = %L where event_id = %L', :'user2ID', :'eventID'),
    format('host user %s not found in community', :'user2ID'),
    'Should fail when updating event_host to user from different community'
);
