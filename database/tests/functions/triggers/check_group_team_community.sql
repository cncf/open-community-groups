...
-- Should succeed when team member is from same community as group
select lives_ok(
    format('insert into group_team (group_id, user_id, role) values (%L, %L, %L)', :'groupID', :'user1ID', 'organizer'),
    'Should succeed when team member is from same community as group'
);
