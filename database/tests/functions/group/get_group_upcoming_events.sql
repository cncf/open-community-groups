...
\set allianceID '6a030000-0000-0000-0000-000000000001'
...
select is(
    get_group_upcoming_events(:'allianceID'::uuid, 'test-group', array['in-person', 'virtual', 'hybrid'], 10)::jsonb,
    jsonb_build_array(
        get_event_summary(:'allianceID'::uuid, :'groupID'::uuid, :'event2ID'::uuid)::jsonb
    ),
    'Should return published non-test future events ordered by date ASC as JSON'
);
...
select is(
    get_group_upcoming_events(:'allianceID'::uuid, 'non-existing-group', array['in-person', 'virtual', 'hybrid'], 10)::jsonb,
    '[]'::jsonb,
    'Should return empty array with non-existing group slug'
);
...
select is(
    (
        ...
        from jsonb_array_elements(
            get_group_upcoming_events(:'allianceID'::uuid, 'test-group', array['virtual'], 10)::jsonb
        ) event_item
    ),
...
select is(
    get_group_upcoming_events(:'allianceID'::uuid, 'test-group-pretty', array['virtual'], 10)::jsonb,
    jsonb_build_array(
        get_event_summary(:'allianceID'::uuid, :'groupID'::uuid, :'event2ID'::uuid)::jsonb,
