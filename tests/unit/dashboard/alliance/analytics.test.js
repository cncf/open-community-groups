...
import {
  initializeAllianceAnalyticsFromPage,
  initAnalyticsCharts,
} from "/static/js/dashboard/alliance/analytics.js";
import { resetDom } from "/tests/unit/test-utils/dom.js";

describe("dashboard alliance analytics", () => {
  const originalEcharts = globalThis.echarts;
  let setOptionCalls;

  const getAllianceAnalyticsPayload = () => ({
    groups: {
      running_total: [
        [1, 1],
        [2, 2],
  ...
  it("initializes the default groups analytics tab", async () => {
    ...
    await initAnalyticsCharts(getAllianceAnalyticsPayload());
  ...
  it("initializes charts from the page payload only once", async () => {
    ...
    marker.dataset.allianceAnalytics = "";
    marker.textContent = JSON.stringify(getAllianceAnalyticsPayload());
    document.body.append(marker);

    // Run the page initializer twice to verify duplicate renders are guarded.
    await initializeAllianceAnalyticsFromPage();
    await initializeAllianceAnalyticsFromPage();
  ...
  it("initializes charts when analytics content is swapped by htmx", async () => {
    ...
    expect(setOptionCalls).to.have.length(8);
    expect(marker.dataset.allianceAnalyticsReady).to.equal("true");
  });
