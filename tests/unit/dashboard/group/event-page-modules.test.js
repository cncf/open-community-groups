...
describe("event page modules", () => {
  ...
  it("does not bind duplicate update page handlers when initialized twice", async () => {
    // Mount the update page shell.
    mountUpdatePageShell({ canManageEvents: true });
    ...
    // Click the publish button after initializing twice.
    document.getElementById("publish-event-button").click();
    await waitForMicrotask();
  ...
  it("ignores stale dashboard context intercepts after a save request", () => {
    // Mount and initialize the update page before an intercepted response.
    mountUpdatePageShell({ canManageEvents: true });
    const refreshEvents = [];
    document.getElementById("event-update-page").addEventListener("refresh-event-submissions", () => {
      refreshEvents.push("page");
    });
    initializeEventUpdatePage();

    // Dispatch a 204 that the server intercepted without running the handler.
    document.getElementById("update-event-button").dispatchEvent(
      new CustomEvent("htmx:afterRequest", {
        bubbles: true,
        detail: {
    ...
    // The intercept is not treated as a successful save.
  ...
  it("restores the stashed add-page section on the update page", async () => {
    ...
    expect(document.querySelector('[data-section="details"]').getAttribute("data-active")).to.equal("false");
  });
  ...
  it("activates a stashed lazy section through its tab button after publishing", () => {
    ...
    expect(document.querySelector('[data-content="attendees"]').classList.contains("hidden")).to.equal(false);
  });
  ...
  it("clears the stashed section when the editor follow-up GET fails", async () => {
    ...
    expect(document.querySelector('[data-section="details"]').getAttribute("data-active")).to.equal("true");
    expect(document.querySelector('[data-section="date-venue"]').getAttribute("data-active")).to.equal(
      "false",
    );
  });
