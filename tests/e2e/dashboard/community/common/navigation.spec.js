...
test.describe("community dashboard navigation", () => {
  ...
  test("admin can switch communities and restore the original selection", async ({
    adminCommunityPage,
  }) => {
    ...
    const communitySelectorButton = adminCommunityPage.locator(
    ...
    try {
      ...
      await waitForActionResponse(
        adminCommunityPage,
        () => secondaryCommunity.click(),
        {
          method: "PUT",
          urlEndsWith: `/dashboard/community/${TEST_COMMUNITY_IDS.community2}/select`,
        },
      );
      await expect(communitySelectorButton).toContainText(
        TEST_COMMUNITY_TITLE_2,
      );
      await expect(
        adminCommunityPage.getByRole("button", { name: "Add Group" }),
      ).toBeDisabled();
    } finally {
