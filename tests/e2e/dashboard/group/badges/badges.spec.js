...
test.describe("group badge definitions", () => {
  test("empty state guides the first badge definition", async ({ organizerEmptyGroupPage }) => {
    ...
    await expect(dashboardContent.getByRole("button", { name: "Add badge" })).toBeVisible();
  });

  test("organizer can move between badge definition result pages", async ({ organizerGroupPage }) => {
  ...
  test("badge management is available only to authorized roles", async ({
    ...
    // Verify direct badge tab requests are forbidden to viewers.
  ...
  test("organizer can search, create, edit, and delete a badge", async ({ organizerGroupPage }) => {
    ...
    const badgeName = uniqueName("Lifecycle Badge");
    const updatedBadgeName = `${badgeName} Updated`;
    ...
    try {
      ...
      // Search for the seeded Speaker badge.
      ...
      // Clear the badge search and wait for the list to reset.
      ...
      // Fill and submit the new badge definition.
      ...
      // Verify the created badge appears in the refreshed list.
      ...
      // Update the badge definition through the edit dialog.
      ...
      // Remove temporary badge definitions if the UI cleanup did not finish.
      if (!deletedThroughUi) {
        cleanupBadgeDefinitionsByName([badgeName, updatedBadgeName]);
      }
    }
  });
});

/** Deletes named badge definitions from the badge table. */
const cleanupBadgeDefinitionsByName = (names) => {
  queryE2eDatabase(`
    delete from badge
    where name in (${names.map((name) => `'${name.replaceAll("'", "''")}'`).join(", ")});
  `);
};
