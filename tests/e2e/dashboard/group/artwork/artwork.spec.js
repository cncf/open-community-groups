...
test.describe("group badge artwork", () => {
  ...
  test("organizer cannot remove artwork referenced by a definition", async ({
    organizerGroupPage,
  }) => {
    // Open the gallery and target the artwork used by the Host definition.
    await navigateToPath(organizerGroupPage, ARTWORK_PATH);
    const artworkItem = organizerGroupPage
      .locator('li:has(img[src*="7744970f"])')
      .first();
  ...
  test("organizer can remove unreferenced artwork", async ({
    organizerGroupPage,
  }) => {
    // Open the gallery and select the dedicated unreferenced fixture.
    await navigateToPath(organizerGroupPage, ARTWORK_PATH);
    const artworkItem = organizerGroupPage.locator(
      'li:has(img[src$="e2e-removable-badge.png"])',
    );
    ...
    await expect(
      organizerGroupPage.getByText("3 items", { exact: true }),
    ).toBeVisible();
  });

  test("organizer can upload artwork and save it to the library", async ({
    organizerEmptyGroupPage,
  }) => {
    ...
    const dashboardContent =
      organizerEmptyGroupPage.locator("#dashboard-content");
    const galleryItems = dashboardContent.locator(
      "li:has(img[src^='/images/badges/'])",
    );
    ...
    const artworkUrl = await organizerEmptyGroupPage
      .locator(
        'image-field[name="badge_artwork_url"] input[name="badge_artwork_url"]',
      )
      .inputValue();
    ...
    const newArtworkItem = dashboardContent.locator(
      `li:has(img[src$="${artworkFileName}"])`,
    );
    ...
    await newArtworkItem
      .getByRole("button", { name: /Remove artwork/u })
      .click();
    await waitForActionResponse(
      organizerEmptyGroupPage,
      () =>
  ...
  test("artwork uploads explain invalid formats and oversized files", async ({
    organizerGroupPage,
  }) => {
    // Load the direct-upload artwork field before exercising server errors.
    await navigateToPath(organizerGroupPage, ARTWORK_PATH);
    const artworkField = organizerGroupPage.locator(
      'image-field[name="badge_artwork_url"]',
    );
    ...
    await expect(formatAlert).toContainText(
    ...
    await expect(valueInput).toHaveValue("");
    await expect(
      organizerGroupPage.getByRole("button", { name: "Save to library" }),
    ).toBeDisabled();
  });

  test("artwork form requires an uploaded image", async ({
    organizerGroupPage,
  }) => {
    ...
    await expect(
      organizerGroupPage.getByRole("heading", { name: "Badges Artwork" }),
    ).toBeVisible();
  });
});
