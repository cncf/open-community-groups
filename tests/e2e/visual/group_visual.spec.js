...
import { getIntroSection, navigateToGroup } from "../utils.js";
...
test.describe("group page visual regression @visual", () => {
  test("matches desktop snapshot", async ({ page }, testInfo) => {
    ...
    await expectRegionScreenshot(page, getIntroSection(page), "group-page-desktop.png", { testInfo });
  });
  ...
  test("matches mobile snapshot @mobile", async ({ page }, testInfo) => {
    ...
    await expectRegionScreenshot(page, getIntroSection(page), "group-page-mobile.png", {
      testInfo,
