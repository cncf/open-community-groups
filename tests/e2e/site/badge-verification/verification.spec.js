...
test.describe("public badge verification page", () => {
  ...
  test("verifies active and revoked credential references", async ({ page }) => {
    ...
    // Verify the active credential resolves to its badge details.
    ...
    // Verify the revoked credential resolves to its badge details.
  ...
  test("verifies an authenticated exported PNG", async ({ member1Page }) => {
    ...
    // Verify the export request succeeds before uploading its bytes.
    ...
    // Verify the exported PNG resolves to its badge details.
  ...
  test("rejects an invalid credential reference", async ({ page }) => {
    ...
    // Verify invalid references show the public failure alert.
    await expect(page.getByRole("alert")).toContainText(
      "This badge could not be verified as an OCG-issued credential.",
    );
  });
